...
dt_af_exhbs_wiinfo <- join(dt_af_exhbs, dt_af_instns, on = c(InstitutionID = "ID")) %>%
    .[, begin_year := year(BeginDate)]

dt_af_exhbs_wiinfo[, .(nbr_type = fnunique(InstitutionID), nbr_exhbs = .N), InstitutionType] %>%
    .[, `:=`(perc_type = 100*nbr_type/sum(nbr_type), perc_exhbs = 100*nbr_exhbs/sum(nbr_exhbs))] %>%
    .[order(-nbr_exhbs)] %>% print(n=25)
```
...
- see if it affects artists as well: 


```{r, fig.dim = c(9, 8), echo = F}
## artfacts exhb with people info 
dt_af_exhbs_wpinfo <- join(dt_af_links_exhb_ppl, dt_af_exhbs_wiinfo, on = c(ExhibitionID = "ID"))
    
dt_af_exhbs_wpinfo[begin_year %between% c(1990, 2022)] %>%
    .[, .(nbr_unq_people = fnunique(PeopleID), nbr_people = .N), .(InstitutionType, begin_year)] %>%
    .[, .SD[any(nbr_people > 350)], InstitutionType] %>% 
    melt(id.vars = c("InstitutionType", "begin_year"), variable.name = "measure", value.name = "count") %>% 
    ggplot(aes(x=begin_year, y = count, color = InstitutionType)) +
    geom_line() +
...
- number of institutions that have at least 1 show (instn_unq_cnt)

- average number of exhibitions by active institutions (instns_avg_exhbcnt)

- average number of artists exhibitions (artist_avg_exhbcount)


possible explanations: 
- data collection: AF has become more and more lazy since 2012 
  but weird that it's so gradual.. 
  they didn't give us good data
...
- rather than count of exhibitions, percent of total exhibitions? 
  this has the funny property that it shrinks if exhibition count stays the same, but more museums are added
