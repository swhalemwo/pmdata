...
copy(dt_af_qntlprep) %>%
    .[, quantile := ecdf_fun(N, N), .(CountryName, begin_year)] %>% 
    .[!is.na(PMDB_ID) & begin_year <= 2020, .SD[fnunique(PMDB_ID) > 5], CountryName] %>% # focus on countries with largest PMs population
    ggplot(aes(x=begin_year, y=quantile, group = PMDB_ID)) +
    geom_line(alpha = 0.2) +
