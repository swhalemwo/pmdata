...
## visualize quantile development of PMs
copy(dt_af_qntlprep)[CountryName == "Austria"][, quantile := ecdf_fun(N, log(N+1)), .(CountryName, begin_year)] %>%
    .[!is.na(PMDB_ID), .SD[fnunique(PMDB_ID) > 5], CountryName] %>% # focus on countries with largest PMs population
    .[, .SD[fnunique(begin_year) > 1], PMDB_ID] %>% # focus only on PMs that have exhbs in more than 1 year
