...
t_gwd_apecprn <- function(dt_acpe_w_id1, ARTNEWS_APECPRN_FILE) {
    ...
    res <- F
    res <- fnrow(dt_ape_cprn[!dt_apecprn_ff, on = .(clctr_name, clctr_name2)]) == 0
...
#' generate the artnews data.table with ACE, ACPE, and APE (link entries to persons)
...
#' generate dt of unique artnews person with their APE ID, calls gd_artnews_collector_person (which is where both arguments are passed to)
...
gd_artnews_person <- function(
    ...
    dt_artnews_collector_person <- gd_artnews_collector_person(
        ARTNEWS_COLLECTOR_PERSON_FILE_ORG,
        ARTNEWS_APECPRN_FILE)
...
t_gwd_pmdb_founder_person <- function(
    ...
    ## read back from file, check that 
...
#' generate and/or test the PMDB person comparison file
#' identify founder-persons that are actually the same
#' PPE: PMDB person entity
#' pmdb_ppecprn_file:
#' @param dt_pmdb_founder_person_wid the data.table with founder_id, founder_person_id and pmdb_person_id, generated by gd_pmdb_founder_person
#' @param PMDB_PPECPRN_FILE the file of comparisons of similar names, read to compare with dt_pmdb_founder_person_wid
t_gwd_ppecprn <- function(dt_pmdb_founder_person_wid,
    ...
    ## dt_pmdb_founder_person[, .(clctr_name = founder_name)] %>% t_subset


    ## with PPE (PMDB Person entity)
    ## dt_pmdb_founder_person_wppe <- copy(dt_pmdb_founder_person) 
        


    ## dt_pmdb_founder_person[, .N, founder_name][N > 1]
...
#' provide the overall data.table for PMDB matches with founder_id, founder_person_id, and pmdb_person_id
#' in particular, it adds pmdb_person_id
...
gd_pmdb_founder_person <- function(
    ...
    ## read in the PMDB_PPECPRN_FILE
    ## update names with update join

    ## assign IDs, based on unique names

    ## fix wrong IDs with update join
