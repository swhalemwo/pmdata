...
links between MOW and PMDB are created through manual fuzzy string matching in emacs with the consult framework ("match_pmdb_mow.el" provides the function, "mow-museums.el" is the list of MOW museums so that they can be imported into emacs) and R ("match_pmdb_mow.R").

This produces "mow_pmdb_match_res.csv": a list of PMDB ID and where available, the correponding MOW ID.
